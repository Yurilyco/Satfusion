...
from src.losses import mse_loss, mae_loss, ssim_loss, tv_loss, Shift , ergas, sam ,D_lambda, D_s, qnr,fre_loss
from src.modules import BicubicUpscaledBaseline, OurFramework
import pandas as pd
import os
...
class LitModel(pl.LightningModule):
    ...
    def check_loss_functions_weights_are_valid(self):
        ...
        # print(self.hparams.w_mse, self.hparams.w_mae, self.hparams.w_ssim, self.hparams.w_tv, self.hparams.w_sam)
        # print(self.hparams.w_mse + self.hparams.w_mae + self.hparams.w_ssim + self.hparams.w_tv + self.hparams.w_sam)
        # sum_weights = self.hparams.w_mse + self.hparams.w_mae + self.hparams.w_ssim + self.hparams.w_tv
        sum_weights = self.hparams.w_mse + self.hparams.w_mae + self.hparams.w_ssim + self.hparams.w_tv + self.hparams.w_sam
    ...
    def create_metrics(self):
        ...
        self.baseline_train_metrics = self.baseline_metrics_factory(
            "train", self.baseline_shifter
        )
        self.baseline_val_metrics = self.baseline_metrics_factory("val", self.baseline_shifter)
        self.baseline_test_metrics = self.baseline_metrics_factory("test", self.baseline_shifter)
    ...
    def metrics_factory(self, prefix, shifter):
        ...
        """
        return {
        f"{prefix}/MSE": lambda y_hat, y, x, pan: shifter.registered_loss(mse_loss)(y_hat, y),
        f"{prefix}/MAE": lambda y_hat, y, x, pan: shifter.registered_loss(mae_loss)(y_hat, y),
        f"{prefix}/SSIM_loss": lambda y_hat, y, x, pan: shifter.registered_loss(ssim_loss)(y_hat, y, window_size=5),
        f"{prefix}/TV": lambda y_hat, y=None, x=None, pan=None: tv_loss(y_hat),  # TV只需要y_hat
        f"{prefix}/ERGAS": lambda y_hat, y, x, pan: ergas(y_hat, y),
        f"{prefix}/SAM": lambda y_hat, y, x, pan: sam(y_hat, y),
        f"{prefix}/D_lambda": lambda y_hat, y, x, pan:D_lambda(y_hat, x),
        f"{prefix}/D_s": lambda y_hat, y, x, pan: D_s(y_hat, x, pan),
        f"{prefix}/QNR": lambda y_hat, y, x, pan: qnr(y_hat, x, pan),
        }
    def baseline_metrics_factory(self, prefix, shifter):
        """Creates a dictionary of metrics that use the shifter when computing the losses (MSE, MAE, SSIM. TV) for the train, validation and test sets.
        ...
        f"{prefix}/SSIM_loss": lambda y_hat, y, x, pan: shifter.registered_loss(ssim_loss)(y_hat, y, window_size=5),
        f"{prefix}/TV": lambda y_hat, y=None, x=None, pan=None: tv_loss(y_hat),  # TV只需要y_hat
        f"{prefix}/ERGAS": lambda y_hat, y, x, pan: shifter.registered_loss(ergas)(y_hat, y),
        f"{prefix}/SAM": lambda y_hat, y, x, pan: shifter.registered_loss(sam)(y_hat, y),
        }
    ...
    def loss(self, batch, metrics, baseline_metrics, prefix):
        ...
        # if self.use_artificial_dataset:
        #     i = 0  # 例如，取出第 0 个元素（索引从 0 开始）,与单独使用Sharpening时取出的图像索引保持一致
        #     reference_x = x[:, i, :, :, :] # 取出第 1 维的第 i 个元素
        # else:
        reference_x = F.interpolate(
            y,
            size=(50, 50),  # 目标尺寸 (h, w)
            mode='bilinear',  # 插值模式：'nearest', 'bilinear', 'bicubic' 等
            align_corners=False  # 对齐选项（对双线性/双三次插值有效）
        )
        m_y_hat, mse_y_hat, mae_y_hat, ssim_y_hat, tv_y_hat, sam_y_hat = self.compute_metrics(y, y_hat, metrics, prefix,
                                                                                              reference_x, pan)
        ...
        baseline_m = self.compute_baseline_metrics(
            y, y_hat_base, m_y_hat, baseline_metrics, prefix, reference_x, pan
        )

        if misr_out != None and sharpening_out != None:  # MISR + Sharpening
            #print("MISR + Sharpening")
            misr_out = misr_out[:, 0]
            m_misr_out, mse_misr_out, mae_misr_out, ssim_misr_out, tv_misr_out, sam_misr_out = self.compute_metrics(y, misr_out, metrics, prefix, reference_x, pan)
            sharpening_out = sharpening_out[:, 0]
            m_sharpening_out, mse_sharpening_out, mae_sharpening_out, ssim_sharpening_out, tv_sharpening_out, sam_sharpening_out = self.compute_metrics(y, sharpening_out, metrics, prefix, reference_x, pan)
            ...
            loss = (
                    ...
                    + (self.hparams.w_ssim * ssim_sharpening_out)
                    + (self.hparams.w_sam * sam_y_hat)
            )
            #特殊测试，整体损失
            '''
            loss = (
                    ...
                    + (self.hparams.w_mae * mae_y_hat)
                    + (self.hparams.w_tv * tv_y_hat)
                    + (self.hparams.w_ssim * ssim_y_hat)
                    + (self.hparams.w_sam * sam_y_hat)
            )
            '''
            #特殊测试，整体损失：分阶段监督
            ...
            loss_misr_out = (
                    (self.hparams.w_mse * mse_misr_out)
                    + (self.hparams.w_mae * mae_misr_out)
                    + (self.hparams.w_tv * tv_misr_out)
                    + (self.hparams.w_ssim * ssim_misr_out)
                    + (self.hparams.w_sam * sam_misr_out)
            )
            loss_sharpening_out = (
                    (self.hparams.w_mse * mse_sharpening_out)
                    + (self.hparams.w_mae * mae_sharpening_out)
                    + (self.hparams.w_tv * tv_sharpening_out)
                    + (self.hparams.w_ssim * ssim_sharpening_out)
                    + (self.hparams.w_sam * sam_sharpening_out)
            )
            loss = (
                loss_y_hat * 0.4
                + loss_misr_out * 0.3
                + loss_sharpening_out * 0.3
            )
            '''
        ...
        if self.hparams.benchmark:
            self.validation_log(y, detach_dict(m_y_hat), baseline_m, prefix)
    ...
    def compute_metrics(self, y, y_hat, metrics, prefix, x, pan):
        ...
        """
        metrics = {
            metrics_function: metrics[metrics_function](y_hat, y,x,pan)
        ...
        sam = metrics[f"{prefix}/SAM"].clone()
        metrics[f"{prefix}/PSNR"] = -10.0 * torch.log10(mse)
        return metrics, mse, mae, ssim, tv ,sam

    def compute_baseline_metrics(
        self, y, y_hat_base, metrics, baseline_metrics, prefix, x , pan
    ):
        ...
        """
        baseline_m = {k: baseline_metrics[k](y_hat_base, y,x , pan) for k in baseline_metrics}
        ...
        ####
        loss = (
            (self.hparams.w_mse * baseline_m[f"{prefix}/MSE"])
            + (self.hparams.w_mae * baseline_m[f"{prefix}/MAE"])
            + (self.hparams.w_tv * baseline_m[f"{prefix}/TV"])
            + (self.hparams.w_ssim * baseline_m[f"{prefix}/SSIM_loss"])
        )
        baseline_m[f"{prefix}/loss"] = loss
    ...
    # 添加一个函数，计算baseline_metrics
    def unpack_and_reduce_baseline_metrics(self, loss_output, prefix):
        metrics = loss_output["baseline_metrics"]
        metrics_reduced = {
            metric_name:metric.mean() for metric_name, metric in metrics.items()
        }
        return metrics, metrics_reduced
    ...
    def predict_step(self,batch,batch_idx):
        ...
        #y_hat = self.bias_adjust(y, y_hat)
        # loss_output = self.loss(
        #     batch, self.predict_metrics, self.baseline_predict_metrics, prefix="predict"
        # )
        # _, metrics_reduced = self.unpack_and_reduce_metrics(loss_output, prefix="predict")
        # metrics_reduced_prefixed = {f"predict/{k}": v for k, v in metrics_reduced.items()}
        #self.log_dict(metrics_reduced_prefixed)
    ...
    def test_step(self, batch, batch_idx):
        ...
        _, baseline_metrics_reduced = self.unpack_and_reduce_baseline_metrics(loss_output, prefix="test")

        # 为 metrics_reduced 添加前缀
        metrics_reduced_prefixed = {f"model/{k}": v for k, v in metrics_reduced.items()}

        # 为 baseline_metrics_reduced 添加前缀
        baseline_metrics_reduced_prefixed = {f"baseline/{k}": v for k, v in baseline_metrics_reduced.items()}

        # 合并字典并记录到 WandB
        combined_metrics = {**metrics_reduced_prefixed, **baseline_metrics_reduced_prefixed}
    ...
    def add_model_specific_args(parent_parser):
        ...
        # 使用人造数据集时，可以适当加入噪声
        parser.add_argument("--temporal_noise", default=0.00, type=float)
        parser.add_argument("--temporal_jitter", default=0.00, type=float)
...
class ImagePredictionLogger(pl.Callback):
    ...
    def _on_epoch_end(self, prefix, batch, trainer, pl_module):
        ...
        if self.use_artificial_dataset:
            ...
            lr = self.generator.generate(hr_real)
            hr = hr_real
            hr_pan = hr_pan_real
        else :
            lr, hr , hr_pan= batch["lr"], batch["hr"], batch["hr_pan"]
        ...
        lr, hr, sr, sr_base, hr_pan = self.extract_patches_from_images(
            sr, sr_base, lr, hr, hr_pan, window_size, window_size_hr
        )
    ...
    def extract_patches_from_images(
        self, sr, sr_base, lr, hr, hr_pan, window_size, window_size_hr
    ):
        ...
        return lr, hr, sr, sr_base, hr_pan
    ...
    def on_test_epoch_end(self, trainer, pl_module):
        ...
        pl_module : pytorch_lightning.LightningModule
            The model.
        """
        max_batches_to_upload = 25  # 只上传前 20 个批次的数据
