...
def make_dataloaders(
    ...
    **kws,
    ...
    dict of torch.utils.data.DataLoader
        The dataloaders.
    """
    #print(root,number_of_revisits,file_postfix,use_tifffile_as_reader,transforms.keys(),chip_size,chip_stride)
    (
        root,
        number_of_revisits,
        file_postfix,
    ...
    #print(root, number_of_revisits, file_postfix, use_tifffile_as_reader, chip_stride)
    ...
    #print(satellite_datasets_arguments)
    ## TODO(ori): extract
    ...
    if type(datasets) is dict:
        ...
        # print(number_of_scenes_per_split)      #{'train': 8, 'val': 1, 'test': 1}
    ...
    # 可以看一下dataloader里面的内容
    # first_batch = next(iter(train_dataloader))  # 取出第一个 batch，每一个batch是一个字典，字典里面是一个Tensor值
    # print("First batch contents:")
    # for key, value in first_batch.items():
    #     print(f"  {key}: {value.shape}")
    '''First batch contents:
  lr: torch.Size([4, 8, 3, 50, 50])
  lrc: torch.Size([4, 8, 1, 50, 50])
  hr: torch.Size([4, 1, 3, 156, 156])
  hr_pan: torch.Size([4, 1, 1, 156, 156])'''
...
def generate_chipped_filtered_and_augmented_datasets(
    ...
    number_of_scenes = len(datasets)
    #print((number_of_scenes, chip_size, chip_stride, input_size))
...
def transpose_scenes_and_chips(dataset, number_of_chips, number_of_scenes):
    ...
    # print(dataset[0])
    # print("dataset[0][lr]" ,dataset[0]['lr'].shape)           # dataset[0][lr] torch.Size([8, 3, 50, 50])
    # print("dataset[0][lrc]", dataset[0]['lrc'].shape)         # dataset[0][lrc] torch.Size([8, 1, 50, 50])
    # print("dataset[0][hr]", dataset[0]['hr'].shape)           # dataset[0][hr] torch.Size([1, 3, 156, 156])
    # print("dataset[0][hr_pan]", dataset[0]['hr_pan'].shape)   # dataset[0][hr_pan] torch.Size([1, 1, 156, 156])
...
def load_dataset_JIF(**kws):
    ...
    **kws : dict
    ...
    kws.setdefault("scene_classification_filter_values", (1,))
    kws.setdefault("scene_classification_filter_threshold", 0.8)
    kws.setdefault("scene_classification_to_color", False)
...
def set_train_test_val_split(kws, root):
    ...
    number_of_aois = len(kws['list_of_aois'])
    print("number_of_aois:",number_of_aois)
