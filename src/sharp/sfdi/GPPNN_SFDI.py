...
import numpy as np
from .modules_sfdi import InvertibleConv1x1
from .refine_sfdi import Refine
import torch.nn.init as init
from src.utils.CDC_SFDI import cdcconv
...
class InvBlock(nn.Module):
    ...
    #    y2 = x2.mul(torch.exp(self.s)) + self.G(y1)  # 2 channel  upd on 3.27
        self.s = self.clamp * (torch.sigmoid(self.H(y1)) * 2 - 1).clamp(-10, 10)  # 限制范围
        y2 = x2.mul(torch.exp(self.s.clamp(-5, 5))) + self.G(y1)  # 限制exp输入
...
class Freprocess(nn.Module):
    ...
    def forward(self, msf, panf):
        ...
        _, _, H, W = msf.shape
        msF = torch.fft.rfft2(self.pre1(msf).float()+1e-8, norm='backward')
        panF = torch.fft.rfft2(self.pre2(panf).float()+1e-8, norm='backward')
        ...
        out = torch.abs(torch.fft.irfft2(out, s=(H, W), norm='backward'))+1e-8
...
class FeatureProcess(nn.Module):
    def __init__(self, channels):
        ...
        self.conv_p = nn.Conv2d(3, channels, 3, 1, 1)
        ...
        self.fuse = nn.Conv2d(3*channels,channels,1,1,0) ## 5->3
    ...
    #    msout = self.fuse(torch.cat([msf0,msf1,msf2,msf3,msf4],1)) upd 3.29
        msout = self.fuse(torch.cat([msf0,msf1,msf2],1))
...
class GPPNN(nn.Module):
    ...
    #    mHR = upsample(ms, M, N) UPD ON 3.24
        mHR = ms 
